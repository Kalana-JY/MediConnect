        ...
        <div className="flex gap-1 bg-white rounded-xl p-1 shadow-sm border border-[#e8edf2] mb-6">
          {tabs.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex-1 py-2.5 rounded-lg text-[13.5px] font-medium border-none cursor-pointer transition-all ${tab === t.key ? 'bg-[#1a6fa0] text-white shadow-sm' : 'bg-transparent text-[#6b7b8d] hover:text-[#1e2a3a]'}`}>
              {t.label}
            </button>
          ))}
        </div>
